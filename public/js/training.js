...
training.start = function(categories, traindata, iterations){
    ...
    traindata.forEach(function(d){
        ...
        net_train_data.push(new convnetjs.Vol([
            d.lat, d.lon, d.temp,
            date.getMonth()+1, date.getDay(), date.getHours(), date.getMinutes()]));
    ...
    function finishedBatch() {
        ...
        // training.log("training: " + categories[net_train_labels[predicted_label]] + " / " + JSON.stringify(predicted_label_soft));
        ...
        if (training.iterations === 0) {
            ...
            $.ajax({
                type: "POST",
                url: '/train',
                data: JSON.stringify(magicNet.toJSON()),
